...
export async function runDeliveryManager(ctx: AgentExecutionContext): Promise<AgentResult> {
  ...
  try {
    ...
    const snap = `stage=${ctx.state.stage}; agent=${ctx.state.currentAgent}; blocked=${ctx.state.blocked}; tasks=${taskStats}; queue_lines=${queueSection.split("\n").length}`;
    const filled = interpolateTemplate(tpl, {
      agent_id: "delivery-manager",
      role: agentRole,
      mission: agentMission,
      resolved_context: snap.slice(0, 4000),
...
`;
  ...
  const statusPath = OUTPUT_PATHS[0];
  const summaryPath = OUTPUT_PATHS[1];
